...
### E. Tipografía y Accesibilidad (Type Style)

La legibilidad y el renderizado tipográfico son prioridades de accesibilidad y ergonomía cognitiva.

- **Variables Globales:** Se exponen de forma dinámica `DOTFILE_FONT` y `DOTFILE_FONT_SIZE`.
- **Integración con Ghostty:** Las fuentes se inyectan dinámicamente en el alias de Ghostty para evitar fuentes sobre-escaladas o excesivamente gruesas que dificulten la visualización en pantallas de baja resolución.
- **Ansible & Markdown Dinámico:** El playbook de Ansible y las plantillas de configuración cargan de forma dinámica estas variables a través de `local.yml` y el archivo de entorno centralizado.
...
- [x] **Variables de Entorno:** Exponer el estilo de fuente y tamaño como `DOTFILE_FONT` y `DOTFILE_FONT_SIZE` de forma dinámica para su uso en markdown, shell y ansible.
