...
local function get_customer_context()
    ...
    if path:find(docs_base, 1, true) ~= 1 then
        return "mrgs"
    end

    -- Ensure we didn't just match a directory that starts with the same prefix (e.g., Documents_backup)
    if #path > #docs_base and path:sub(#docs_base + 1, #docs_base + 1) ~= "/" then
        return "mrgs"
    end
    ...
    if first_folder and first_folder ~= "wiki" and first_folder ~= "" then
        return first_folder
    end
...
function M.open_wiki_note()
    ...
    if ctx.wiki_path and vim.fn.filereadable(ctx.wiki_path) == 1 then
        ...
        vim.notify(
            "[Whip] No Wiki note found for ticket " .. ctx.ticket_key .. " in [" .. ctx.customer .. "]",
            vim.log.levels.WARN
        )
    end
end
