        ...
        elseif in_frontmatter then
            ...
            if key then
                ...
                if val ~= "" then
                    if val:match("^%s*%[(.*)%]%s*$") then
                        ...
                        for item in list_content:gmatch("[^,%s]+") do
                            item = item:gsub('^"%s*', ''):gsub('%s*"$', ''):gsub("^'%s*", ''):gsub('%s*\'$', '')
                            if item ~= "" then
                ...
                if list_item and current_key then
                    list_item = list_item:gsub('^"%s*', ''):gsub('%s*"$', ''):gsub("^'%s*", ''):gsub('%s*\'$', '')
                    if list_item ~= "" then
...
function M.scan_vault()
    ...
    if cache.last_scan == 0 then
        if load_cache() then
            ...
            if os.time() - cache.last_scan < 3600 then
                return
            end
        end
    end
    ...
    local now = os.time()
    if cache.cwd == vim.fn.getcwd() and now - cache.last_scan < 3600 then
        return
    end
    ...
    if before_cursor:match("^%s*-%s*") then
        ...
        for r = cursor_row - 1, 1, -1 do
            ...
            if l then
                ...
                if l == "---" or (not l:match("^%s") and not l:match("^%s*-")) then
                    break
                end
            end
        end
...
vim.api.nvim_create_autocmd("BufWritePost", {
    ...
    callback = function(ev)
        M.update_file_in_cache(ev.match)
    end,
})
...
vim.api.nvim_create_autocmd("FileType", {
    ...
    pattern = "markdown",
    callback = function()
        vim.defer_fn(M.scan_vault, 2000)
    end,
})
