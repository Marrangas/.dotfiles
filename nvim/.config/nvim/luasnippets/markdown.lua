...
local wiki_completions = require("util.wiki_completions")

-- Function to get formatted string: YYYY-wWW (e.g., 2026-w32)
local function get_year_week()
...
-- Dynamically queries files and subtags from the vault cache to populate choice node options
local function get_emociones_choices()
    local choices = {
        opt("afectos incluidos", "emociones y sentimientos"),
        opt("otros", "factores no clasificados"),
    }

    -- 1. Query for all notes tagged with "afectos"
    local ok_notes, notes = pcall(wiki_completions.get_notes_by_tag, "afectos")
