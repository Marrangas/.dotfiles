WITH
  ...
  filtered_resource_usage_with_unused AS (
    SELECT
      ...
      IFNULL(SAFE_DIVIDE(filtered_resource_usage.amount, aggregated_resource_usage_by_resource_name.amount), 0) * breakdown_untracked_resource_usage.amount AS allocate_unused,
  ...
  filtered_resource_usage_with_unused_and_unallocated AS (
    SELECT
      ...
      filtered_resource_usage_with_unused.amount + filtered_resource_usage_with_unused.allocate_unused + unallocated_usage.amount * IFNULL(SAFE_DIVIDE(filtered_resource_usage_with_unused.amount, total_used.amount), 0) AS amount_with_untracked,
